...
function Source:_route_to_provider(context_result, provider_ctx, callback, start_time,
  ...
  local Context = require('nvim-ssns.completion.statement_context')
  ...
  if context_result.type == Context.Type.TABLE then
    ...
    if is_cross_db_schema and cross_db_target then
      Debug.log(string.format("[COMPLETION] Routing to SchemasProvider for cross-db: %s",
        context_result.potential_database))
      local SchemasProvider = require('nvim-ssns.completion.providers.schemas')
    ...
    if mode:sub(1, 4) == "join" then
      ...
      local JoinsProvider = require('nvim-ssns.completion.providers.joins')
    ...
    local TablesProvider = require('nvim-ssns.completion.providers.tables')
  ...
  elseif context_result.type == Context.Type.COLUMN then
    ...
    local ColumnsProvider = require('nvim-ssns.completion.providers.columns')
  ...
  elseif context_result.type == Context.Type.PROCEDURE then
    ...
    local ProceduresProvider = require('nvim-ssns.completion.providers.procedures')
  ...
  elseif context_result.type == Context.Type.PARAMETER then
    ...
    local ParametersProvider = require('nvim-ssns.completion.providers.parameters')
  ...
  elseif context_result.type == Context.Type.DATABASE then
    ...
    local DatabasesProvider = require('nvim-ssns.completion.providers.databases')
  ...
  elseif context_result.type == Context.Type.KEYWORD then
    ...
    local KeywordsProvider = require('nvim-ssns.completion.providers.keywords')
    local FunctionsProvider = require('nvim-ssns.completion.providers.functions')
    local SnippetsProvider = require('nvim-ssns.completion.providers.snippets')
