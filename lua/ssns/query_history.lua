...
---Search across all buffer histories
---@param pattern string Search pattern (plain text)
---@param options table? Search options {case_sensitive: boolean, server: string?, database: string?}
---@return table<string, QueryHistoryEntry[]> buffer_id -> matching entries
function QueryHistory.search(pattern, options)
  options = options or {}
  local results = {}
  ...
  for buffer_id, buffer_history in pairs(QueryHistory.buffers) do
    -- Filter by server/database if specified
    if options.server and buffer_history.server_name ~= options.server then
      goto continue
    end
    if options.database and buffer_history.database ~= options.database then
      goto continue
    end

    -- Search entries in this buffer
    local matching_entries = {}
    for _, entry in ipairs(buffer_history.entries) do
      local query = entry.query
      if not options.case_sensitive then
        query = query:lower()
      end

      if query:find(search_pattern, 1, true) then
        table.insert(matching_entries, entry)
      end
    end

    if #matching_entries > 0 then
      results[buffer_id] = matching_entries
    end
