...
local EtlParser = require("ssns.etl.parser")
local Directives = require("ssns.etl.directives")
local EtlContext = require("ssns.etl.context")
local EtlExecutor = require("ssns.etl.executor")

-- Lazy load UI module (avoid circular deps)
local EtlResults
local function get_results_ui()
  if not EtlResults then
    EtlResults = require("ssns.ui.etl_results")
  end
  return EtlResults
end
