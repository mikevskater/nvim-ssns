...
function TokenContext.is_temp_table(identifier)
...
function TokenContext.is_global_temp_table(identifier)
...
function TokenContext.strip_identifier_quotes(identifier)
  if not identifier or type(identifier) ~= "string" or #identifier < 2 then
...
function TokenContext.get_last_name_part(qualified_name)
...
function TokenContext.starts_with(str, prefix)
...
function TokenContext.extract_trailing_bracketed(text)
...
function TokenContext.extract_trailing_identifier(text)
  if not text or type(text) ~= "string" then
    return nil
  end
